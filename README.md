...
- AWS Budgets: Used to monitor AWS spending and send alerts when the cost crosses defined budget thresholds.
 
- Amazon EC2: Used to create compute resources and simulate stopped instances for testing unused resource detection.
  
- Amazon EBS: Used to create storage volumes and demonstrate detection of unattached EBS volumes.
  
- Elastic IP: Used to allocate static public IP addresses and simulate unused Elastic IP resources.
  
- AWS Lambda: Used to automate the detection and cleanup of unused AWS resources such as stopped EC2 instances, unattached EBS volumes, and unused Elastic IP addresses.
  
- Amazon CloudWatch:Used to monitor and store logs generated by the Lambda function during resource cleanup.
   
- AWS IAM: Used to manage permissions and allow the Lambda function to access and control AWS resources securely.
